...
describe('9 - Implemente os casos de teste da função `getCharacter`', () => {
  it('Verifica se a função `getCharacter` retorna o objeto do personagem corretamente.', () => {
    ...
    expect(getCharacter('melissandre') || getCharacter('Melissandre')).toEqual({name: 'Melissandre',class: 'Necromancer',phrases: ['Death By Fire Is The Purest Death.','For The Night Is Dark And Full Of Terrors.']});
    expect(getCharacter('Neita')).toBeUndefined();
    // expect(getCharacter('Brienne')).toBe({name: 'Arya Stark', class: 'Rogue', phrases: [ 'Not today', 'A girl has no name.' ]});
